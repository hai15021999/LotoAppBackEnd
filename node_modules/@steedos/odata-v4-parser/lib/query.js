...
(function (Query) {
    ...
    function selectItem(value, index) {
        ...
        if (op > index) {
            ...
            item = {};
            var name = nameOrIdentifier_1.default.qualifiedEntityTypeName(value, index) ||
            ...
            if (!select_1)
                return;
            index = select_1.next;
            item = name ? { name: name, select: select_1 } : select_1;
        }
        if (index > start)
    ...
    function selectPath(value, index) {
        ...
        if (value[index] === 0x2f) {
            var name = nameOrIdentifier_1.default.qualifiedComplexTypeName(value, index + 1);
            if (name) {
                index = name.next;
